...
public class CalService
{
	...
	public String cal(String text, boolean isPercent) throws Exception
	{
		...
		if (isPercent)
		{
			secondResult = CustomMath.multiply(Double.valueOf(firstNum), CustomMath.divide(secondResult, 100));
		}
		// 四则运算，返回结果赋给第一个操作数
		if (this.lastOp.equals("+"))
		{
			firstNum = String.valueOf(CustomMath.add(Double.valueOf(firstNum), secondResult));
		} else if (this.lastOp.equals("-"))
		{
			firstNum = String.valueOf(CustomMath.sub(Double.valueOf(firstNum), secondResult));
		} else if (this.lastOp.equals("*"))
		{
			firstNum = String.valueOf(CustomMath.multiply(Double.valueOf(firstNum), secondResult));
		} else if (this.lastOp.equals("/"))
		{
			firstNum = String.valueOf(CustomMath.divide(Double.valueOf(firstNum), secondResult));
		}
		// 给第二个操作数重新赋值
	...
	public String setReciprocal(String text)
	{
		...
		} else
		{
			...
			this.isSecondNum = true;
			// 计算结果并返回
			return String.valueOf(CustomMath.divide(1, Double.valueOf(text)));
		}
	}
	...
	public String mCmd(String cmd, String text)
	{
		if (cmd.equals("M+"))
		{
			...
			store = CustomMath.add(store, Double.valueOf(text));
